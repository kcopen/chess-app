import { Chessboard, ChessColor, ChessMove, Piece, PieceType } from "./ChessTypes";
...
export const getBestMove = (team: ChessColor, board: Chessboard): ChessMove | undefined => {
	...
	const possibleMoves: ChessMove[] = scoreMoves(getValidTeamMoves(team, board));
...
function scoreMoves(moves: ChessMove[]) {
	moves.forEach((m) => {
		m.score = scoreMove(m);
	});
	return moves;
